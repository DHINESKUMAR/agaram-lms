...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      setHomework(allHomework.filter((h: any) => h.grade === data.grade));
    };
    loadData();
...
useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans relative">
      ...
      <a 
        href={`https://wa.me/94778054232?text=${encodeURIComponent(`வணக்கம் அகரம் தினேஸ் ஐயா அவர்களே! எனது பிள்ளை பெயர்: ${displayName}, தரம்: ${studentData.grade}, மாவட்டம்: ${studentData.district || 'N/A'}`)}`}
        target="_blank"
        rel="noopener noreferrer"
        className="fixed bottom-24 right-4 z-50 bg-green-500 text-white px-4 py-2 rounded-full shadow-lg hover:bg-green-600 transition-transform hover:scale-105 flex items-center justify-center gap-2 font-bold text-sm"
      ...
      {/* Main Content */}
      <main className={`flex-1 overflow-y-auto p-4 md:p-6 lg:p-8 bg-transparent pb-24 ${activeTab === 'chat' ? 'flex flex-col p-0 md:p-0 lg:p-0' : 'max-w-7xl mx-auto w-full'}`}>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6">
              ...
              <div
                ...
                className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col items-center justify-center cursor-pointer hover:shadow-md hover:-translate-y-1 transition-all group relative sm:col-span-2 md:col-span-2"
              >
                {unreadCount > 0 && (
                ...
                <span className="font-bold text-slate-700 text-sm sm:text-base">Live Chat Support</span>
              </div>
            </div>
